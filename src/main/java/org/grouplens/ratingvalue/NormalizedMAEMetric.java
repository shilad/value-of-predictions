...
public class NormalizedMAEMetric extends MAEPredictMetric {
    private static final Logger logger = LoggerFactory.getLogger(NormalizedMAEMetric.class);
    private static final String[] COLUMNS = { "NMAE", "NMAE.ByUser" };
    private static final String[] USER_COLUMNS = {"NMAE"};
    private PreferenceDomain domain;

    public NormalizedMAEMetric(PreferenceDomain domain) {
        this.domain = domain;
    }
    ...
    class Accum implements TestUserMetricAccumulator {
        ...
        public String[] finalResults() {
            ...
            logger.info("NMAE: {}", normalize(errRate));
            return new String[] {
                    Double.toString(normalize(errRate)),
